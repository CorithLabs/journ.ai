...
function Row({ item, onToggle, onDel, onUpd }: { item: TodoItem; onToggle: () => void; onDel: () => void; onUpd: (t: string) => void }) {
        ...
        {ed
          ...
          : <div>
              ...
              <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                ...
                {item.sourceActivityId && <span className="text-xs text-ink-muted bg-surface-overlay px-2 py-0.5 rounded-full">📍 From itinerary{item.sourceDayIndex !== undefined ? ` · Day ${item.sourceDayIndex + 1}` : ''}</span>}
            ...
            <section key={cat} aria-label={`${cat} tasks`}>
              ...
              <div className="space-y-1.5">
                {items.filter(t => t.status !== 'done').map(item => (
                  <Row key={item.id} item={item} onToggle={() => toggle(item)} onDel={() => del(item)} onUpd={t => upd(item, t)} />
                ))}
                {items.filter(t => t.status === 'done').map(item => (
                  <Row key={item.id} item={item} onToggle={() => toggle(item)} onDel={() => del(item)} onUpd={t => upd(item, t)} />
                ))}
